...
trait SDD extends Circuit[SDD] with Tractable {
  ...
  def respects(vtree: VTree.Some) = (vtree == this.vtree)
  def subRespects(vtree: VTree.Some) = (vtree.containsNode[VTree.Some](this.vtree))
  
  def variables: immutable.Set[Variable] = vtree.variables
...
trait TrueNode extends SDD{
  ...
  override def entailedLiterals = immutable.Set.empty[Literal]
...
trait LiteralNode extends SDD {
  ...
  override def entailedLiterals = immutable.Set(literal)
...
trait DecisionNode[+N <: SDD] extends SDD {
  ...
  def isConsistent = decomp.isConsistent
  ...
  def entailedLiterals = decomp.entailedLiterals
